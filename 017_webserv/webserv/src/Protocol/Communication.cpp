...
/*   Updated: 2024/10/16 13:05:01 by jmartos-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	#pragma region CLIENT
		...
		#pragma region Read
			...
			//	Read some data
				...
				//	No more data coming
					if ((event->read_maxsize > 0 && event->read_size >= event->read_maxsize)) {
						if (event->response_method == "CGI") {
							EventInfo * cgi_event = Event::get(event->cgi_fd);																	//	Get the CGI's event
		...
		#pragma region Write
			...
			//	All data has been sent
				...
				//	Log connection
					...
					if (time.size() > 1 && time[0] == '-') time = time.substr(1);
	...
	#pragma region CGI
				...
				//	Needs to get the header
					if (event->read_info == 0 && event->read_maxsize == 0 && event->header == "") {
						...
						if (result == 0) {																						//	There is a header
							...
							if (Utils::stol(event->header_map["Content-Length"], content_length)) {						//	There is no 'Content-Length'
		...
		#pragma region Write
			...
			//	There are data in the 'write_buffer'
				...
				//	All data has been sent
				if ((event->write_info == 3 && event->write_buffer.empty())) {
